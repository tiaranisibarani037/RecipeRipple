...
<body>
    <div>
        <form action="{{ route('recipe.update', $recipe->id) }}" method="POST" id="recipe-form" enctype="multipart/form-data">
            @csrf
            ...
            <div class="container">
                ...
                <div class="video-placeholder" id="videoIconLabel">
                    @if ($recipe->video)
                        <video id="videoPreview" controls>
                            <source src="{{ asset('storage/videos/' . $recipe->video) }}" type="video/mp4">
                        </video>
                    @else
                ...
                <input type="text" class="form-control" id="name" name="name" placeholder="Judul: ..." value="{{ $recipe->name }}" required>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="kategori_id" name="kategori_id" required>
                        @foreach ($kategori as $kategoriItem)
                            <option value="{{ $kategoriItem->id }}" {{ $kategoriItem->id == $recipe->kategori_id ? 'selected' : '' }}>
                                {{ $kategoriItem->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div id="bahan-container">
                    @foreach ($recipe->bahan as $bahan)
                        <div class="bahan-item">
                            <input type="text" class="form-control" name="bahan[]" value="{{ $bahan }}" placeholder="Bahan...">
                ...
                <div id="langkah-container">
                    @foreach ($recipe->langkah as $index => $langkah)
                        <div class="langkah-item">
                            <input type="text" class="form-control" name="langkah[]" value="{{ $langkah['description'] }}" placeholder="Langkah...">
                            @if (isset($langkah['image']))
                                <img src="{{ asset('storage/images/' . $langkah['image']) }}" class="step-image" id="stepPreview{{ $index + 1 }}">
                            @else
